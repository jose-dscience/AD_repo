...
typedef struct {
    ...
    unsigned int* key_pos; //array containing the position of the nodes given indexes
    unsigned int* rev_pos; //array containing the indexes given nodes
} binheap_type;
...
void swap_keys(binheap_type *H, unsigned int n_a, unsigned int n_b)
{
    ...
    aux = H->key_pos[n_a];
    memcpy(&(H->key_pos[n_a]),&(H->key_pos[n_b]),sizeof(unsigned int));
    memcpy(&(H->key_pos[n_b]),&(aux),sizeof(unsigned int));
