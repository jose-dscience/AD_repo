                   ...
                   const size_t row_start_C, const size_t col_start_C, const size_t n)
{
    ...
    float ***P_mini = (float***)malloc(sizeof(float **) * 7);
    for(size_t i=0; i<7; i++)
        P_mini[i] = allocate_matrix(n2/2, n2/2);
    ...
    //P1 = A11 x S1
    strassen_aux_op(P[0], A, (const float* const* const)S[0], P_mini,
    ...
    //P2 = S2 X B22
    strassen_aux_op(P[1], (const float* const* const)S[1], B, P_mini,
                   0, 0,
                   row_start_B+n2, col_start_B+n2,
                   0, 0, 
                   n2);
    ...
    //P3 = S3 X B11
    strassen_aux_op(P[2], (const float* const* const)S[2], B, P_mini,
                   0, 0,
    ...
    //P4 = A22 X S4
    strassen_aux_op(P[3], A, (const float* const* const)S[3], P_mini,
                   row_start_A+n2, col_start_A+n2,
                   0, 0,
                   0, 0, 
                   n2);
    ...
    //P5 = S5 X S6
    strassen_aux_op(P[4], (const float* const* const)S[4], (const float* const* const)S[5], P_mini,
                   0, 0,
                   0, 0,
                   0, 0, 
                   n2);
    ...
    //P6 = S7 X S8
    strassen_aux_op(P[5], (const float* const* const)S[6], (const float* const* const)S[7], P_mini,
                   0, 0,
                   0, 0,
                   0, 0, 
                   n2);
    ...
    //P7 = S9 X S10
    strassen_aux_op(P[6], (const float* const* const)S[8], (const float* const* const)S[9], P_mini,
                   0, 0,
                   0, 0,
                   0, 0, 
                   n2);
    ...
    for(size_t i=0; i<7; i++)
        deallocate_matrix(P_mini[i], n2/2);
    free(P_mini);
}

void strassen_aux_generalized_op(float **C, 
                   float const *const *const A, 
                   float const *const *const B,
                   ...
                   const size_t row_start_C, const size_t col_start_C)
{
    ...
    float ***P_mini = (float***)malloc(sizeof(float **) * 7);   //alloc first P matrixes
    for(size_t i=0; i<7; i++)
        P_mini[i] = allocate_matrix(npivot/2, npivot/2);
    
    //Calculation of C11 using pure and gener. strassen algorithm
    strassen_aux_op(C, A, B, P_mini, row_start_A, col_start_A, row_start_B, col_start_B, row_start_C, col_start_C, npivot);
    ...
    deallocate_matrix(D, npivot);
    
    for(size_t i=0; i<7; i++)
        deallocate_matrix(P_mini[i], npivot/2);
    free(P_mini);
}
